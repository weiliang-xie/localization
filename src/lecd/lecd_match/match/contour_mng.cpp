...
// 递归结构 计算各层的统计变量并存储到cont_views_ parent是上一层次的椭圆数据指针 cc_roi的初始值是bev全范围  制作cont_views_
                                                ...
                                                const std::shared_ptr<ContourView> &parent)
{
    ...
    {                                              // n=0: background
                                                   //      printf("Area: %d\n", stats.at<int>(n, cv::CC_STAT_AREA));
        if (stats(n, 4) < cfg_.min_cont_cell_cnt_) // ignore contours that are too small   //面积太小忽略
            continue;
        ...
        RunningStatRecorder tmp_rec;
        int poi_r = -1, poi_c = -1; // the point(r,c) coordinate on the global bev for the contour   //在全局bev下的坐标(像素)
        ...
        std::shared_ptr<ContourView> ptr_tmp_cv(new ContourView(level, poi_r, poi_c)); // 构建新的椭圆指针
        ...
        makeContourRecursiveHelper(rect_g, mask_n, level + 1, ptr_tmp_cv); // 继续递归 这里的递归不是完全处理完一个层次的椭圆后才递归，而是在每个椭圆处理时就进行递归，由下到上
