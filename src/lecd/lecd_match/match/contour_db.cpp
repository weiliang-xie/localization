...
void LayerDB::rebuild(int idx_t1, double curr_ts)
{
    ...
    if (sz1 > sz2)
    {
        ...
        { // if no contagious value across split  回退to_move_mid个的key值是单独的 可以分割
        ...
        else
        {
            ...
            int i = to_move_mid - 1;
            for (; i > to_move_min; i--)
            { // look into larger side
                if (tr1.data_tree_[sort_permu[sz1 - i]](bucket_chann_) != contagious_val)
                { // found the split position 往回寻找单独的key值 直到找到to_move_min
                    num_to_move = i;
        ...
        if (num_to_move == 0)
        { // should rebalance but cannot, due to a strip of contagious bucket values 有连续一致的key值，无法重建
            printf("Cannot split due to contagious values A");
            tr1.popBufferMax(curr_ts); //?直接全部pop进，等下一次重建再分割
            if (pb2)
                tr2.popBufferMax(curr_ts); // 若有，也pop进
            return;
        }

        for (int i = 0; i < num_to_move; i++)
        { // add to shorter tree  调整 balance
                             ...
                             std::vector<std::pair<IndexOfKey, KeyFloatType>> &res_pairs) const
{
    ...
    for (int i = 0; i < max_num_backets_; i++)
    {
        ...
        { // 选择bucket_ranges中的与key(bucket_chann_)相近的index
            mid_bucket = i;
            break;
        }
    }
