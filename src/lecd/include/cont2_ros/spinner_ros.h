...
struct BaseROSSpinner {
  struct GlobalPoseInfo {     //全局位姿数据
    Eigen::Isometry3d T_wl;
    double z_shift{};         //这里分析是每1分钟偏移10 单位：time_translate
...
//  tf2_ros::TransformListener tfListener;
  ...
  explicit BaseROSSpinner(ros::NodeHandle &nh_) : nh(nh_) //, tfListener(tfBuffer)
  {
    ...
    sub_stop_go = nh.subscribe("/cont2_status", 100, &BaseROSSpinner::statusCallback, this);  //接收状态话题
  }

  //状态话题接收回调，根据状态调整（暂停 取消 继续 终止？）
