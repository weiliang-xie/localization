...
// The configuration for checking the similarity of two lecds
struct LECDSimThresConfig {
...
// use a separate recorder to record when creating lecd view, and discard it after use.  一个椭圆内的暂存数据，用于计算椭圆的特性参数
...
//    voxels_pos_.emplace_back(v_rc);  // desc cont itself (4/6)
  ...
  static RunningStatRecorder addLECDStat(const RunningStatRecorder &rec1, const RunningStatRecorder &rec2) {
...
struct LECDView {
  ...
  // config
//  const LECDViewStatConfig cfg_;

  // property:
  int16_t level_;   //lay level 
  int16_t poi_[2]; // a point in full bev coordinate belonging to this lecd/slice. 在bev下的像素坐标？
  ...
  // Raw data (the pixels that belong to this LECD. Is is necessary?)
  // TODO
...
//  // hierarchy
//  std::shared_ptr<LECDView> parent_;
//  std::vector<std::shared_ptr<LECDView>> children_;

  // TODO: 0. build a lecd from 3: pic, roi, height threshold. Called in manager.
  explicit LECDView(int16_t level, int16_t poi_r, int16_t poi_c) : level_(level) {
    DCHECK_GE(poi_r, 0);
    DCHECK_GE(poi_c, 0);
    poi_[0] = poi_r;
    poi_[1] = poi_c;
...
*/
  ...
  inline bool eccentricitySalient(const LECDViewStatConfig &cfg) const {
  ...
  inline bool centerOfMassSalient(const LECDViewStatConfig &cfg) const {
    return (com_ - pos_mean_).norm() > cfg.com_bias_thres;
  }

  // TODO
  bool orietSalient(const LECDViewStatConfig &cfg) const {
    return false;
  }


  // TODO: 3. return true if two lecds can be accepted as from the same heatmap peak
  ...
  // T_tgt = T_delta * T_src
//  static std::pair<Eigen::Isometry2d, bool> checkCorresp(const LECDView &lecd_src, const LECDView &lecd_tgt) {
  //求解统计量的相似度 返回二值量
  static bool checkSim(const LECDView &lecd_src, const LECDView &lecd_tgt,
                       const LECDSimThresConfig &simthres) {
//                       const LECDSimThresConfig &simthres = LECDSimThresConfig()) {
    // very loose
...
//    std::pair<Eigen::Isometry2d, bool> ret(Eigen::Isometry2d(), false);
    ...
    //判断椭圆的面积差距和差异率过大
    if (diff_perc<float>(lecd_src.cell_cnt_, lecd_tgt.cell_cnt_, simthres.tp_cell_cnt)
        && diff_delt<float>(lecd_src.cell_cnt_, lecd_tgt.cell_cnt_, simthres.ta_cell_cnt)) {
#if HUMAN_READABLE
    ...
    if(std::min(lecd_src.eig_vals_(0), lecd_tgt.eig_vals_(0)) > 2.0 && diff_delt<float>(lecd_src.eccen_,lecd_tgt.eccen_, simthres.ta_eccen)){
#if HUMAN_READABLE
    ...
    if (std::max(lecd_src.eig_vals_(1), lecd_tgt.eig_vals_(1)) > 2.0 &&
        diff_perc<float>(std::sqrt(lecd_src.eig_vals_(1)), std::sqrt(lecd_tgt.eig_vals_(1)), simthres.tp_eigval)) {
#if HUMAN_READABLE
    ...
    if (std::max(lecd_src.eig_vals_(0), lecd_tgt.eig_vals_(0)) > 2.0 &&
        diff_perc<float>(std::sqrt(lecd_src.eig_vals_(0)), std::sqrt(lecd_tgt.eig_vals_(0)), simthres.tp_eigval)) {
#if HUMAN_READABLE
    ...
    //网格数量大于15 && 高度均值差距过大       
    if (std::max(lecd_src.cell_cnt_, lecd_tgt.cell_cnt_) > 15 &&
        diff_delt<float>(lecd_src.vol3_mean_, lecd_tgt.vol3_mean_, simthres.ta_h_bar)) {
#if HUMAN_READABLE
...
//     //加权质心和中心的距离的差距过大
//     const float com_r1 = (lecd_src.com_ - lecd_src.pos_mean_).norm();
//     const float com_r2 = (lecd_tgt.com_ - lecd_tgt.pos_mean_).norm();
...
//     }
  ...
  // TODO: 4. add two lecds. Only statistical parts are useful


  // Add lecd results (**NOT** accurate statistics!)
  // procedure: revert to stat recorder and merge
  //添加轮廓结果，利用过程记录器并合并
  static LECDView
  addLECDRes(const LECDView &cont1, const LECDView &cont2, const LECDViewStatConfig &cfg) {
    ...
    LECDView res(cont1.level_, cont1.poi_[0], cont1.poi_[1]);
...
//  void addChildren(std::shared_ptr<LECDView> &chd) {
//    children_.push_back(chd);
//  }

  // auxiliary functions
  // 1. get the position of all lecd pixels
  std::vector<std::vector<int>> getContPixelPos() const {
    return {};
  }

//  // 2. visualize lecd/slice
//  void displayLECD(const std::string &fpath) const {
//
//  }
