...
/// Find the nearset neighbour in a sorted data vector 查询最接近的 返回对应的序号
/// \tparam T
...
int lookupNN(const T &q_val, const std::vector<T> &sorted_vec, const T &tol) {
  auto it_low = std::lower_bound(sorted_vec.begin(), sorted_vec.end(), q_val);    //返回查询的点云帧序号在点云序号集合里的迭代器
  auto it = it_low;
